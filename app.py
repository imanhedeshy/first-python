...
def add_user():
    ...
    if not name or not email:
        return jsonify({"error": "Name and email are required fields", "success": False}), 400

    with app.app_context():
        # Check if the user with the same email already exists
        existing_user = User.query.filter_by(email=email).first()
        if existing_user:
            return jsonify({"error": "User with this email already exists", "success": False}), 400
        ...
        return jsonify({"message": "User added successfully", "success": True, "user": {"id": new_user.id, "name": new_user.name, "email": new_user.email} }), 201
